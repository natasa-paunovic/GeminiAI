...
class ChatRepository(
    ...
    suspend fun fetchHistoryMsg(chatId: String, title: String) {
        if (title == "New chat") {
            try {
                ...
                if (msg.isNotEmpty()) {
                    val chatTitle = chat.sendMessage("Give me one short title for this content $msg")
    ...
    suspend fun deleteChat(chatId: String) {
       val messageRef= chatRef.document(chatId)
            .collection(MESSAGE_SUB_COLLECTION)
        val messages= messageRef.get().await()

        for (message in messages.documents) {
            messageRef.document(message.id).delete().await()
        }

        // Finally, delete the chat document
        chatRef.document(chatId).delete().await()
    }
