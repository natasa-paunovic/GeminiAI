...
class AiNavigationActions(navController: NavController) {

    val navigateToForgotPasswordScreen:() -> Unit= {
        navController.navigateToSingleTop(Route.ForgotPasswordScreen().route)
    }

    val navigateToLoginScreenWithArgs:(isEmailVerified:Boolean) -> Unit= {
        navController.navigate(Route.LoginScreen().getRouteWithArgs(isEmailVerified = it)){
